...
class NoName(nn.Module):
    def __init__(self, config, eps=0.2, noise_scale = 0.1, noise_min = 0.0001, noise_max = 0.02, steps = 5, beta_fixed=True):
        ...
        self.encoder2 = ConvTransE(self.n_ent, self.n_rel,self.d_model, self.dropout_rate, self.dropout_rate, self.dropout_rate)
    ...
    def forward(self, heads, rels, tails, year, month, day):
        ...
        rels_embeds_img = self.encoder2.get_rel_embedding(rels)
    ...
    def train_forward(self, heads, rels, tails, year, month, day, neg):
        ...
        condition_emb2 = self.encoder2(heads_embs2 - rels_embs1, d_img)
        ...
        ent_embs1 = 1 / self.sqrt_alphas_cumprod[ts].unsqueeze(1).unsqueeze(1) * ent_embs1 - \
                        torch.sqrt(1 / self.alphas_cumprod[ts].unsqueeze(1).unsqueeze(1) - 1) * self.denoiser1(ent_embs1, time_embs.unsqueeze(1), condition_emb1.unsqueeze(1))
        ...
        type_intes = (self.dropout(condition_emb1.multiply(x_start1).unsqueeze(1) \
                                  ...
                                  - condition_emb2.unsqueeze(1).multiply(ent_embs2)).sum(dim=-1)) /2
        #type_intes = (torch.norm(condition_emb - x_start, dim=1).unsqueeze(1) - torch.norm(condition_emb.unsqueeze(1) - ent_embeds, dim=-1))
        loss_lp = self.link_prediction_loss(type_intes, ent_type, torch.zeros_like(tails))
    ...
    def test_forward(self, sub, rels, tails, year, month, day, local_weight=1.):
        ...
        while t >= 0:
          ...
          time_embs = self.time_mlp(time_embs)
          x_t1 = 1 / self.sqrt_alphas_cumprod[t] * (x_t1 - self.betas[t] / self.sqrt_one_minus_alphas_cumprod[t] * \
                                              self.denoiser1(x_t1.unsqueeze(1), time_embs.unsqueeze(1), condition_emb1.unsqueeze(1)).squeeze(1))
          x_t2 = 1 / self.sqrt_alphas_cumprod[t] * (x_t2 - self.betas[t] / self.sqrt_one_minus_alphas_cumprod[t] * \
                                              self.denoiser2(x_t2.unsqueeze(1), time_embs.unsqueeze(1), condition_emb2.unsqueeze(1)).squeeze(1))
          if t > 0:
            x_t1 += 0.1 * torch.randn_like(condition_emb1)
            x_t2 += 0.1 * torch.randn_like(condition_emb2)
          t -=  1
        ...
        condition_emb1 = (heads_embs1 + rels_embs1) * condition_emb1
        condition_emb2 = (heads_embs2 - rels_embs2) * condition_emb2
        scores = (self.dropout(condition_emb1.multiply(x_t1).unsqueeze(1)).sum(dim=-1)\
